...
/*   Updated: 2024/09/10 22:28:13 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (cmd->flags & reverse) {
		...
		for (int i = 0; i < node->size_childs.n_lines; i++) {
			for (int j = 0; i + j < node->nb_childs; j += node->size_childs.n_lines) {
				ls_display_file(
				    cmd, node->childs[i + j], &node->size_childs,
...
void ls_display(Command *cmd, File *node) {
	...
	if (!node->size_childs.n_lines) node->size_childs.n_lines = 1;
	node->size_childs.n_cols = node->nb_childs / node->size_childs.n_lines;
	if ((double) (node->nb_childs) / (double) (node->size_childs.n_lines)
